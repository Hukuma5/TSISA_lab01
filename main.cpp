...
std::vector<std::pair<double, double>> opt_passive(const double& a, const double& b) {
    ...
    while (delta > pogr) {
        ...
        for (size_t k = 1; k <= N; ++k) {
            vec_y.push_back(function((b - a) / (N + 1) * k + a));
        }
...
std::vector<double> fib(size_t N, double& a, double& b) {
    ...
    cout <<"Погрешность равна "<<(b-a) <<endl;
    return values;
}
...
void PrintValues(const std::vector<double>& values) {
    ...
    cout << std::string(45, '-') << endl;
    cout << values[values.size() - 1]<<endl<<endl;
}
...
int main() {
    ...
    cout << "Вариант №6:" << endl;
    cout << "Функция (1-x)^2 + exp(x) для интервала [-5, 2]" << endl << endl;
    ...
    cout << "Применим этот метод при N от 7 до 10 "<< endl;
